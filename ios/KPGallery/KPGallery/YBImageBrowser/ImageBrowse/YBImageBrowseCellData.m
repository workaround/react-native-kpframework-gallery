...
static CGFloat _globalZoomScaleSurplus = 2;
...
- (CGSize)getSizeOfCompressing {
    ...
    if (size.width > 16000 && size.width > size.height) {
        // 需要调整到16000的位置，否则ImageView无法识别
        CGFloat maxScale = 16000.0 / imageViewsize.width;
        size = CGSizeMake(16000.0, imageViewsize.height * maxScale);
    }
    else if (size.height > 16000 && size.height >= size.width) {
        // 需要调整到16000的位置，否则ImageView无法识别
        CGFloat maxScale = 16000.0 / imageViewsize.height;
        size = CGSizeMake(imageViewsize.width * maxScale, 16000);
    }
