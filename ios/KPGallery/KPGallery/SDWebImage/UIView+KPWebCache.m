...
- (void)sd_internalSetImageWithURL:(nullable NSURL *)url
    ...
    if (url) {
        ...
        if ([context valueForKey:SDWebImageExternalCustomManagerKey]) {
            manager = (KPSDWebImageManager *)[context valueForKey:SDWebImageExternalCustomManagerKey];
        } else {
        ...
        id <KPSDWebImageOperation> operation = [manager loadImageWithURL:url options:options progress:combinedProgressBlock completed:^(UIImage *image, NSData *data, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
...
- (void)sd_setImage:(UIImage *)image imageData:(NSData *)imageData basedOnClassOrViaCustomSetImageBlock:(SDSetImageBlock)setImageBlock transition:(KPSDWebImageTransition *)transition cacheType:(SDImageCacheType)cacheType imageURL:(NSURL *)imageURL {
    UIView *view = self;
...
- (KPSDWebImageTransition *)sd_imageTransition {
    return objc_getAssociatedObject(self, @selector(sd_imageTransition));
}

- (void)setSd_imageTransition:(KPSDWebImageTransition *)sd_imageTransition {
